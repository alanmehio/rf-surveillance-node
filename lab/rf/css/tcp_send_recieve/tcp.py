...
def chksum(packet: bytes) -> int:
    if len(packet) % 2 != 0:
        ...
        '''
        In C language string is nothing but an array of char type.
        It stores each of the characters in a memory space of 1 byte. Each array is terminated with '\0' or null character
        '''
        packet += b'\0' # this is null character in c see

    res = sum(array.array("H", packet)) # class array.array(typecode[, initializer]) ; "H" int in python min size is 2 bytes
                                        # mapped to unsigned short in C type. see https://docs.python.org/3/library/array.html
...
if __name__ == '__main__':
    ...
    val1:int = chksum(b'Alan Mehio')
    val2:int = chksum(b'Alan Mehio')
   # print(str(val1))
   # print(str(val2))
